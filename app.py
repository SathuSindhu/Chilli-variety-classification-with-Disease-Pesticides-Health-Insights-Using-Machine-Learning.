...
# Load Excel data using raw string literals to handle file paths
chilli_diseases_df = pd.read_excel(r'C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\chilli_diseases.xlsx')
diseases_pesticides_df = pd.read_excel(r'C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\diseases_pesticides.xlsx')
health_effects_df = pd.read_excel(r'C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\health_effects.xlsx')

# Load the trained model using raw string literals for the path
rf_model = joblib.load(r'C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\model.pkl')
img_size = (150, 150)  # Resize to (150, 150) for consistency with model input
...
def preprocess_new_image(image_path, img_size):
    ...
    if image is not None:
        image_resized = cv2.resize(image, img_size)  # Resize image to match model input size
        image_normalized = image_resized / 255.0  # Normalize image to range [0, 1]
        image_flattened = image_normalized.flatten().reshape(1, -1)  # Flatten the image to 1D
...
def upload_image():
    ...
    if preprocessed_image is not None:
        ...
        cv2.imwrite(static_image_path, original_image)
        ...
        if not pesticide_row.empty:
            ...
            for pesticide in pesticides_used:
                ...
                if not health_row.empty:
                    health_effects = health_row.iloc[0]['Health Effects']
...
# # Load Excel data
# chilli_diseases_df = pd.read_excel('C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\chilli_diseases.xlsx')
# diseases_pesticides_df = pd.read_excel('C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\diseases_pesticides.xlsx')
# health_effects_df = pd.read_excel('C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\health_effects.xlsx')


# # Load the trained model
# rf_model = joblib.load('C:\Users\somani\Downloads\chilli-variety-classifier-web-application-main\model.pkl')  # Load the saved Random Forest model
# img_size = (150, 150)
